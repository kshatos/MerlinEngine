...
namespace Merlin
{
    ...
    public:
        Mesh() = default;
        Mesh(
            const VertexType* vertices, size_t vertex_count,
        ...
        inline void SetVertexCount(size_t count)
        {
        ...
        inline uint32_t& GetIndex(uint32_t triangle_index, uint32_t vertex_offset)
        {
        ...
        inline const uint32_t& GetIndex(uint32_t triangle_index, uint32_t vertex_offset) const
        {
            return m_indices[3 * triangle_index + vertex_offset];
        }
