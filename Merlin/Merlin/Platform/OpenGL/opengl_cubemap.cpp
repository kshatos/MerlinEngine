...
namespace Merlin
{
    ...
    OpenGLCubemap::OpenGLCubemap(const std::vector<std::string>& face_paths)
    {
        ...
        for (unsigned int i = 0; i < face_paths.size(); i++)
        {
            ...
            unsigned char* data = stbi_load(
                face_paths[i].c_str(), &width, &height, &channel_count, 0);
            ...
            {
                ME_LOG_ERROR("Cubemap texture failed to load at path: " +
                             face_paths[i]);
            }
    ...
    OpenGLCubemap::~OpenGLCubemap() { glDeleteTextures(1, &m_id); }

    void OpenGLCubemap::Bind(uint32_t slot) { glBindTextureUnit(slot, m_id); }
